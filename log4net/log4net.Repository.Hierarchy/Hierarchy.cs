...
namespace log4net.Repository.Hierarchy
{
	...
	public class Hierarchy : LoggerRepositorySkeleton, IBasicRepositoryConfigurator, IXmlRepositoryConfigurator
	{
		...
		internal class LevelEntry
		{
			...
			{
				return "LevelEntry(Value=" + m_levelValue.ToString() + ", Name=" + m_levelName + ", DisplayName=" + m_levelDisplayName + ")";
			}
		}
			...
			for (int num = name.LastIndexOf('.', length - 1); num >= 0; num = name.LastIndexOf('.', num - 1))
			{
				...
				else
				{
					...
					else
					{
						...
						LogLog.Error(source, "Unexpected object type [" + (((object)type != null) ? type.ToString() : null) + "] in ht.", new LogException());
					}
				}
				if (num == 0)
				{
