...
namespace log4net.Util
{
	...
	public sealed class OptionConverter
	{
		...
		public static object ConvertStringTo(Type target, string txt)
		{
			...
			MethodInfo method = target.GetMethod("Parse", new Type[1]
			{
				typeof(string)
			});
			if (method != null)
			{
				return method.Invoke(null, BindingFlags.InvokeMethod, null, new object[1]
				{
					txt
		...
		public static string SubstituteVariables(string value, IDictionary props)
		{
			...
			throw new LogException("[" + value + "] has no closing brace. Opening brace at position [" + num2.ToString() + "]");
		}
