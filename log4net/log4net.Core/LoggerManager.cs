	...
	public sealed class LoggerManager
	{
		...
		static LoggerManager()
		{
			...
			if (appSetting != null && appSetting.Length > 0)
			{
				...
				{
					LogLog.Error(declaringType, "Exception while resolving RepositorySelector Type [" + appSetting + "]", exception);
				}
				if (type != null)
				{
					object obj = null;
					...
					{
						LogLog.Error(declaringType, "Exception while creating RepositorySelector [" + type.FullName + "]", exception2);
					}
					...
					{
						LogLog.Error(declaringType, "RepositorySelector Type [" + type.FullName + "] is not an IRepositorySelector");
					}
				}
			}
